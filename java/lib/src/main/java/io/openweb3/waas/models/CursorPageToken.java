...
public class CursorPageToken {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CursorPageToken
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CursorPageToken.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CursorPageToken is not found in the empty JSON string", CursorPageToken.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CursorPageToken.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CursorPageToken` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CursorPageToken.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayitems.size(); i++) {
        Token.validateJsonElement(jsonArrayitems.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CursorPageToken.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CursorPageToken' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CursorPageToken> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CursorPageToken.class));

       return (TypeAdapter<T>) new TypeAdapter<CursorPageToken>() {
           @Override
           public void write(JsonWriter out, CursorPageToken value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CursorPageToken read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of CursorPageToken given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of CursorPageToken
   * @throws IOException if the JSON string is invalid with respect to CursorPageToken
   */
  public static CursorPageToken fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CursorPageToken.class);
  }

  /**
   * Convert an instance of CursorPageToken to an JSON string
   *
   * @return JSON string
   */
